...
class PodLauncher(LoggingMixin):
    def __init__(self, kube_client=None, in_cluster=True, cluster_context=None,
        ...
        self.kube_req_factory = pod_fac.ExtractXcomPodRequestFactory(
        ) if extract_xcom else pod_fac.SimplePodRequestFactory()
    ...
    def _extract_xcom(self, pod):
        ...
        self.log.info(resp)
        commands = [
            'cat {}/return.json'.format(self.kube_req_factory.XCOM_MOUNT_PATH),
            'kill -s SIGINT 1'
        ]
        result = None
        while resp.is_open():
